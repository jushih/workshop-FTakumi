...
assert(TheWorld.ismastersim, "Takumi Hunter should not exist on client")
...
local _dirt_prefab = "takumi_dirtpile"
local _track_prefab = "takumi_track"
local _beast_prefab_summer = "koalefant_summer"
local _beast_prefab_winter = "koalefant_winter"
local _alternate_beasts = { "warg", "spat" }

local beast_spawn = "koalefant_summer"
local beast_name = "elephant"
...
local function StartDirt(hunt,position)
	...
	--determine beast at the start (Takumi mod)
	if math.random () <= GetAlternateBeastChance() then
		beast_spawn = GetRandomItem(_alternate_beasts)
	elseif  TheWorld.state.iswinter then
		beast_spawn = _beast_prefab_winter
		beast_name = " an elephant"
	else
		beast_spawn = _beast_prefab_summer
		beast_name = "an elephant"
	end
	
	if beast_spawn == "warg" then
		beast_name = "a wolf"
	elseif beast_spawn == "spat" then
	beast_name = "a sheep"
	end
...
local function SpawnHuntedBeast(hunt, pt)
    ...
    local spawn_pt = GetSpawnPoint(pt, TUNING.HUNT_SPAWN_DIST, hunt)
    if spawn_pt ~= nil then
        hunt.huntedbeast = SpawnPrefab(beast_spawn)
...
function self:OnDirtInvestigated(pt, doer)
	...
	--find track
	if hunt.trackspawned == 0 and hunt.activeplayer.prefab == "takumi" then
		doer.components.talker:Say("This is " .. beast_name  .."'s track.")
--		hunt.activeplayer.components.talker:Say("This is " + beast_spawn + "'s track.")
	end
